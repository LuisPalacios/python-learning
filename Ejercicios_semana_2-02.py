...
def crearConcesionario():
  ...
  while True:
    ...
    sTipo=input("   --- Tipo: ")
    dVehículos[sMatrícula] = { 'Marca':sMarca, 'Precio':sPrecio, 'Tipo':sTipo }

  # Si tengo datos suficientes ...
  if ( len(dVehículos) ):
    # Una vez creado el fichero del concesionario y lo añado a principal
    creaFicheroConcesionarioYAñadeAFicheroPrincipal(sNombre, sCiudad, dVehículos)
...
def consultaConcesionario(sNombreConcesionario):
  ...
  for dMatrícula in dConcesionario['Vehículos']:
    ...
    print(" -> " + dMatrícula + " - Marca: " + dVehículo['Marca'] + " - Precio: " + str(dVehículo['Precio']) + " - Tipo: " + dVehículo['Tipo'] )
  ...
  for sNombreConcesionario in dPrincipal: 
    ...
    for sMatrícula in dPrincipal[sNombreConcesionario]['Vehículos']:
      ...
      if ( menorValor==-1 or dVehículo['Precio'] < menorValor ): 
        ...
        sFound = str( " -> " + sNombreConcesionario + " en " + sCiudad + ": " + sMatrícula + " - " + " - Marca: " + dVehículo['Marca'] + " - Precio: " + str(dVehículo['Precio']) + " - Tipo: " + dVehículo['Tipo'] )
